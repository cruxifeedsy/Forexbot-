...
# ====== GET MARKET DATA ======
def get_data(pair="EURUSD=X", period="1d", interval="1m"):
    df = yf.download(tickers=pair, period=period, interval=interval)
    df.dropna(inplace=True)
    return df

# ====== SIGNAL CALCULATION (always returns a signal) ======
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("pair_"):
        ...
        df = get_data(pair=f"{pair}=X", interval="1m")
        signal = analyze_signal(df)
        probability = calculate_probability(df, signal)
        volatility = "Moderate"  # Placeholder, can enhance later
